	...
	<div class="Hui-article">
		<div style="margin-left:150px;">
			<div class="pd-20">
				<div class="text-c">
				<form id="lotteryForm">
					<input type="text" name="title" id="" placeholder="新闻标题关键字" style="width:250px" class="input-text" value="${newsbTitle}">
					<a class="btn btn-success" onclick="lotterySubmit('news/queryback.do')"><i class="Hui-iconfont">&#xe665;</i>搜索</a>
				</form>
				</div>
				<div class="cl pd-5 bg-1 bk-gray mt-20">
				<span class="l"><a href="back/news/news-add.jsp" class="btn btn-success radius"><i class="Hui-iconfont">&#xe6e2;</i>添加</a></span>
				...
				<div class="mt-20">
						...
						<tbody>
							<c:forEach items="${news}" var="news">
								<tr class="text-c va-m">
									...
									<td>${news.adddate}</td>
									<td>${news.content}</td>
									<td><a href="javascript:del(${news.newsid})">删除</a><a href="back/news/news-update.jsp?title=${news.title}&content=${news.content}">修改</a></td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
					...
					<form id="pageForm">
						...
						<input type="hidden" name="title" value="${newsbTitle}"/>
					</form>
					<jsp:include page="/pageBar.jsp">
						<jsp:param value="news/queryback.do" name="url"/>
					</jsp:include>
				</div>
			</div>
		</div>
...
function del(id) {
	if (confirm('确定要删除！')) {
		newsSubmit("news/delete.do?newsid="+id);
	}
}
