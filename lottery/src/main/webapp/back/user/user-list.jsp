...
<head>
	...
	<script src="js/userbAjax.js" type="text/javascript" charset="utf-8"></script>
  </head>
	...
	<div class="Hui-article">
		<div style="margin-left:150px;">
			<div class="pd-20">
				<div class="text-c">
				<form id="userForm">
					<input type="text" name="username" placeholder="用户名" style="width:250px" class="input-text" value="${username}">
					<input type="text" name="phone" placeholder="电话号码" style="width:250px" class="input-text" value="${phone}">
					<a class="btn btn-success" onclick="submit('user/query.do')"><i class="Hui-iconfont">&#xe665;</i>搜索</a>
				</form>
				</div>
				...
				<div class="mt-20">
						...
						<tbody>
							<c:forEach items="${userList}" var="user">
								<tr class="text-c va-m">
									...
									<td>${user.userid}</td>
									<td>${user.username}</td>
									<td class="text-l">${user.truename}</td>
									<td class="text-l">${user.phone}</td>
									<td class="text-l">${user.anumber}</td>
									<td>${user.logindate}</td>
									<td>${user.registerdate}</td>
									<td>${user.balance}元</td>
									<td>${user.status eq 0?'正常':'冻结中'}</td>
									<td>${user.status eq 0?'':'<a href="javascript:;">解冻</a>'}</td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
					...
					<jsp:include page="/pageBar.jsp">
						<jsp:param value="user/query.do" name="url"/>
					</jsp:include>
				</div>
			</div>
		</div>
