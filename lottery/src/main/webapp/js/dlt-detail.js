...
//    点击篮球动画效果
    ...
    if ($(this).prop("className") == "active2") {   //判断其类名
    	if (bball_nums < 2) {
    		bnumber+=(num+',');   //加‘,’确保替换的时候是正确的
		} else {
			alert('蓝球个数已够!');
			$(this).toggleClass("active2");
		}
    } else {
    	bnumber = bnumber.replace((num+','),'');
    }
    ballNums();
});
/*
...
function redRand() {
    ...
    //输出0～33之间的随机整数
    var rrandom_integer = parseInt(Math.random() * 35 + 1);
...
function blueRan() {
    ...
    obj = Math.floor(Math.random() * 12 + 1);
    if (obj < 10) {
        obj = '0' + obj;
    }
    arr_blue.push(obj);
...
//        console.log(arr_b);
        ...
        $('#buyno').val(rnumber.replace(reg, '')+bnumber.replace(reg, ''));
        $('#jump').submit();
    } else {
    	if (rball_nums != 5 && bball_nums != 2) {
            var error = "您选了（" + rball_nums + "红 +" + bball_nums + "蓝）,个数错误,请选5个红球,2个蓝球";
            alert(error);
        }
